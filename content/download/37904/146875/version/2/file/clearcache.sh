...
TEMPLATE_BLOCK_CACHEFILE="$DIR/cache/template-block"
...
if [ "$CLEAR_TEMPLATE_BLOCK" -eq 1 ]; then
    if [ -d "$TEMPLATE_BLOCK_CACHEFILE" ]; then
	echo "Removing expiry cache file $TEMPLATE_BLOCK_CACHEFILE"
	rm -rf "$TEMPLATE_BLOCK_CACHEFILE"
    fi
fi
